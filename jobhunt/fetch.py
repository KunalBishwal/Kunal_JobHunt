...
class Job:
    ...
    company_priority: str | None = None   # "S", "A", "B"
...
def parse_smartrecruiters(slug: str, company: str, body: Any) -> list[Job]:
    ...
    for j in (body or {}).get("content", []):
        ...
        out.append(Job(
            ...
            description=strip_html(j.get("refNumber") or ""),
...
def fetch_board(ats: str, slug: str, company: str | None = None,
    ...
    if ats not in ENDPOINTS or not slug:
        return []
...
def fetch_all(companies: Iterable[dict], sleep: float = 0.25) -> list[Job]:
    ...
    active_companies = [c for c in companies if c.get("active", True) and c.get("ats") in ENDPOINTS and c.get("slug")]
    print(f"  polling {len(active_companies)} active boards (out of {len(list(companies))} companies in universe)")
