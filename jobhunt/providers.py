...
class AnthropicProvider(Provider):
    ...
    def _client(self):
        try:
            ...
            return anthropic_mod.Anthropic(api_key=self._env("ANTHROPIC_API_KEY"))
        except ImportError:
...
DEFAULT_MODELS = {
    ...
    "groq": {"screen": "llama-3.1-8b-instant", "draft": "llama-3.1-8b-instant"},
...
def resolve(stage: str, check: bool = True) -> tuple[Provider, str]:
    ...
    Precedence: stage-specific env var -> global env var -> detected from active API key.
    """
    name = (os.getenv(f"{stage.upper()}_PROVIDER") or os.getenv("LLM_PROVIDER") or "").strip().lower()

    if not name:
        if (os.getenv("GEMINI_API_KEY") or "").strip():
            name = "gemini"
        elif (os.getenv("GROQ_API_KEY") or "").strip():
            name = "groq"
